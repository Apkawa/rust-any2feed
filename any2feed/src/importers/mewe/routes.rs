...
pub fn route_opml(importer: & MeweImporter) -> Route {
    ...
    Route::new("/mewe/feed.opml",
               move |r| {
                   ...
                   let groups_outlines = mewe_api.fetch_groups().unwrap()
                       .confirmed_groups
                       .iter()
                   ...
                   let users_outlines = mewe_api.get_contacts(true).unwrap()
                       .iter()
                       .map(|g| Outline::new(g.name.as_str())
    ...
    Route::new(
        ...
        move |r| {
            ...
            let mewe_feeds: Vec<MeweApiFeedList> = if let Some(next_page) = page_url {
                // Паджинация
                mewe_api.fetch_feeds(next_page.as_str(), None, None).unwrap()
            } else {
                if pairs[1] != Some("me") {
                ...
                match pairs[1..=2] {
                    ...
                    [Some("user"), Some(_id)] => mewe_api.get_user_feed(user_id.unwrap().as_str(), limit, pages).unwrap(),
...
pub fn route_media_proxy(importer: & MeweImporter) -> Route {
    ...
    Route::new(
        ...
        move |r| {
            ...
            let media_res = mewe_api.get(format!("https://mewe.com/{path}").as_str()).unwrap();
