...
fn test_post_to_entry() {
    let mut json: MeweApiPost = serde_json::from_str(
        fixture::load_json_fixture("post_media").as_str()
    ).unwrap();
...
#[test]
fn test_allfeed() {
    let mut json: MeweApiFeedList = serde_json::from_str(
        fixture::load_json_fixture("allfeed").as_str()
    ).unwrap();
