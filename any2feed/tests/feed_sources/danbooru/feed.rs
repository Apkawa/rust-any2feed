...
fn test_entry_list_generic() {
    let p = serde_json::from_str::<Vec<DanbooruPost>>(
        load_json_fixture(format!("danbooru/post_list").as_str()).as_str(),
    )
    .unwrap()
    ...
    let p = p
        ...
        .collect();
    let e = booru_posts_to_feed(p, None);
    dbg!(e);
}
